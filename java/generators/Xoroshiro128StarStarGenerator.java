 ...
 *
 */
public class Xoroshiro128StarStarGenerator implements RandomGenerator64 {
	 ...
	 */
	public static final int SEED_SIZE = Xoroshiro128StarStarGenerator.STATE_SIZE;
	...
	public Xoroshiro128StarStarGenerator() {
		this(RandomGenerator.DEFAULT_SEED_GENERATOR.generateBytes(Xoroshiro128StarStarGenerator.SEED_SIZE));
	}
	 ...
	 */
	public Xoroshiro128StarStarGenerator(final byte[] seed) {
		this.setSeed(seed);
	}
	 ...
	 *                      A generator.
	 */
	public Xoroshiro128StarStarGenerator(final Xoroshiro128StarStarGenerator generator) {
	...
	public int getSeedSize() {
		return Xoroshiro128StarStarGenerator.SEED_SIZE;
	}

	@Override
	public int getStateSize() {
		return Xoroshiro128StarStarGenerator.STATE_SIZE;
	}
	...
	public void setState(final byte[] state) {
		if (state.length < Xoroshiro128StarStarGenerator.STATE_SIZE) {
	...
	@Override
	public Xoroshiro128StarStarGenerator split() {
		final Xoroshiro128StarStarGenerator generator = new Xoroshiro128StarStarGenerator(this);
		generator.jump();
		return generator;
	}
	...
	public void jump() {
		long state0 = 0;
		long state1 = 0;
		for (int i = 0; i < Xoroshiro128StarStarGenerator.JUMP.length; ++i) {
			for (int j = 0; j < 64; ++j) {
				if ((Xoroshiro128StarStarGenerator.JUMP[i] & (1L << j)) != 0) {
					state0 ^= this.state[0];
					state1 ^= this.state[1];
				}
	...
	public void longJump() {
		long state0 = 0;
		long state1 = 0;
		for (int i = 0; i < Xoroshiro128StarStarGenerator.LONG_JUMP.length; ++i) {
			for (int j = 0; j < 64; ++j) {
				if ((Xoroshiro128StarStarGenerator.LONG_JUMP[i] & (1L << j)) != 0) {
					state0 ^= this.state[0];
					state1 ^= this.state[1];
				}
				this.generateUniformLong();
