 ...
 *
 */
public class Xoshiro512PlusGenerator implements RandomGenerator64 {
	 ...
	 */
	public static final int SEED_SIZE = Xoshiro512PlusGenerator.STATE_SIZE;
	 ...
	 * seed generator.
	 */
	public Xoshiro512PlusGenerator() {
		this(RandomGenerator.DEFAULT_SEED_GENERATOR.generateBytes(Xoshiro512PlusGenerator.SEED_SIZE));
	}
	 ...
	 */
	public Xoshiro512PlusGenerator(final byte[] seed) {
		this.setSeed(seed);
	}
	 ...
	 *                      A generator.
	 */
	public Xoshiro512PlusGenerator(final Xoshiro512PlusGenerator generator) {
	...
	public int getSeedSize() {
		return Xoshiro512PlusGenerator.SEED_SIZE;
	}

	@Override
	public int getStateSize() {
		return Xoshiro512PlusGenerator.STATE_SIZE;
	}
	...
	public void setState(final byte[] state) {
		if (state.length < Xoshiro512PlusGenerator.STATE_SIZE) {
	...
	@Override
	public Xoshiro512PlusGenerator split() {
		final Xoshiro512PlusGenerator generator = new Xoshiro512PlusGenerator(this);
		generator.jump();
		return generator;
	}
	...
	public long generateUniformLong() {
		final long result = this.state[0] + this.state[2];
	...
	public void jump() {
		...
		long state6 = 0;
		long state7 = 0;
		for (int i = 0; i < Xoshiro512PlusGenerator.JUMP.length; ++i) {
			for (int j = 0; j < 64; ++j) {
				if ((Xoshiro512PlusGenerator.JUMP[i] & (1L << j)) != 0) {
					state0 ^= this.state[0];
					state1 ^= this.state[1];
					state2 ^= this.state[2];
					state3 ^= this.state[3];
